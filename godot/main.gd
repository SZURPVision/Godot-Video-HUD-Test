...
func _ready() -> void:
	...
	if log_file:
		# Added Rendering and Total Local Latency to logs
		log_file.store_line("Timestamp,FPS,DecodingLatency_MS,RenderingLatency_MS,TotalLocalLatency_MS,InputLatency_MS,Sender_TS,Sender_Frame")

	_toggle_mouse_capture(true)

	udp_latency_val.text = "N/A"
...
func _process(delta: float) -> void:
	...
	# 3. Total Local Latency (Your requested formula)
	# Video_Latency = Decoding + Rendering
...
func _log_data_to_files():
	if log_file:
		...
		# Log all metrics including the new Total Local calculation
		var line = "%s,%s,%.2f,%.2f,%.2f,%.2f,%.3f,%d" % [
			timestamp,
			fps_val.text,
